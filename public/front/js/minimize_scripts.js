var IconToSvg = function() {
    var _ = { folderPath: '/ready_uploads/svg/' };
    _.init = function() {};
    _.getHtml = function(icon, classes, styles) {
        if (!IsIE11()) {
            ...
            icon = $.trim(icon);
            var url = 'https://static1.s123-cdn-static-a.com/ready_uploads/svg/search.svg?v=2';
        ...
        } else { return '<i class="fa site123-image-icon fa-' + icon + ' ' + classes.replace('svg-m', '') + '" alt="' + icon + '"></i>'; }
    };
    _.getBrandFolder = function(url) {
        var brandFlolder = '';
        if (url.indexOf('brand-1') != -1) { brandFlolder = 'brand-1/'; }
        return brandFlolder;
    }
        ...
        that.$inputs.on('focus', function() {
            ...
            if (!window.isGoogleRecaptchaLoaded) {
                ...
                $.getScript('https://www.google.com/recaptcha/api.js?render=6Lck3r0ZAAAAAOFc__oZANv72nZ3K29O-qsOIYPp').done(function() {
                    grecaptcha.ready(function() { if (isUsingMagicButtons) { $('.grecaptcha-badge').addClass('grecaptcha-badge-new-position'); } });
                });
            } else { $('.grecaptcha-badge').removeClass('hide'); }
        });
        that.$inputs.on('focusout', function() { $('.grecaptcha-badge').addClass('hide'); });
    }
    that.getToken = function() {
        try {
            grecaptcha.ready(function() {
                grecaptcha.execute('6Lck3r0ZAAAAAOFc__oZANv72nZ3K29O-qsOIYPp', { action: 'users_forms_submit' })
                    .then(function(token) {
...
S123.inIframe = function() { try { return window.self !== window.top; } catch (e) { return true; } }();
...
initS123QueryString();
S123.escapeHtml = function(text) { if (!text) return text; var map = { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&apos;' }; return text.toString().replace(/[&<>"']/g, function(m) { return map[m]; }); };
S123.contextmMenuDisable = function() {
    return;
    $('body').on('contextmenu', '.disable-context-menu', function() { return false; });
};
S123.objectAssign = function(target, sources) {
    if (Object.assign) { sources = Object.assign(target, sources); } else {
            ...
            if (IsIE11()) {
                ...
                $('.s123-icon-converter').each(function() {
                    ...
                    if ($this.closest('#top-menu-mobile').length > 0) {
                        ...
                        if ($this.attr('style').indexOf('display') != -1) { $newIcon.css({ display: $this.css('display') }); }
                        $this.replaceWith($newIcon);
                    }
                });
            } else { _.handleFroalaIcons(); }
        });
    };
    _.handleFroalaIcons = function() { loadFiles(); };
    _.getHtml = function(icon, classes, styles) { return IconToSvg.getHtml(icon, classes, styles); };
...
S123.CopyToClipboard = function() {
    var _ = {};
    _.copy = (copyText) => { if (window.isSecureContext && navigator.clipboard) { secureCopy(copyText); } else { unsecuredCopy(copyText); } }
    async function secureCopy(copyText) {
        copyText.select();
        copyText.setSelectionRange(0, 99999);
        try {
            await navigator.clipboard.writeText(copyText.value)
                .then(() => { $.gritter.add({ title: 'Link copied to clipboard', class_name: 'gritter-success', time: 6000 }); })
    ...
    function unsecuredCopy(copyText) {
        ...
        try {
            ...
            $.gritter.add({ title: 'Link copied to clipboard', class_name: 'gritter-success', time: 6000 });
        } catch (err) { console.error('Unable to copy to clipboard', err); }
    }
    return _;
}();
if (typeof menuScrollOffset === 'undefined') { var menuScrollOffset = 0; }
var menuScrollOffset_mobile = 60;
var isMobile = { Android: function() { return navigator.userAgent.match(/Android/i); }, BlackBerry: function() { return navigator.userAgent.match(/BlackBerry/i); }, iOS: function() { return navigator.userAgent.match(/iPhone|iPad|iPod/i); }, Opera: function() { return navigator.userAgent.match(/Opera Mini/i); }, Windows: function() { return navigator.userAgent.match(/IEMobile/i); }, any: function() { return (isMobile.Android() || isMobile.BlackBerry() || isMobile.iOS() || isMobile.Opera() || isMobile.Windows()); } };
var whatScreen = {
    any: function() {
        var screenWidth = $(window).width();
        if (screenWidth <= 544) { return 'mobile'; }
        if (screenWidth > 544 && screenWidth <= 767) { return 'tablet'; }
        if (screenWidth > 768) { return 'desktop'; }
    }
};
...
function DestroyParallaxImages() { $('.parallax-window').parallax('destroy'); }

function RefreshAOS() { $(document).on('s123.page.ready.refreshAOS', function(event) { AOS.refresh(); }); }

function TopSectionInitialize() {
    $(document).on('s123.page.ready', function(event) {
        $.each($('#websitePopupHomeVideo, .promoVideoPopup .iconsCircle, #home_buttonText, #home_buttonText_1, #topAction_buttonText_1, #topAction_buttonText_2, #promoRedirectButton1, #promoRedirectButton2, .s123-widget [data-video-popup="true"]'), function(index, el) { intializePopupVideoPlayers($(el)); });
...
function intializePopupVideoPlayers($el) {
    $el.off('click.intializePopupVideoPlayers').on('click.intializePopupVideoPlayers', function(e) {
        ...
        e.preventDefault();
        e.stopPropagation();
        if (player === 'site123') { videoURL = '/include/globalVideoPlayer.php?cad=1&url=' + encodeURIComponent(videoURL) + '&fluid=true'; }
        if (isMobile.any()) { if (player === 'site123') { videoURL += '&autoplay=false'; } else { videoURL = videoURL.replace('autoplay', 'disable-autoplay'); } }
...
function ContactFormHomeInitialize() {
    $(document).on('s123.page.ready', function(event) {
        if ($('#contactUsFormHome').length !== 0) {
            ...
            customFormMultiSteps.init({ $form: $contactUsFormHome, $nextButton: $contactUsFormHome.find('.next-form-btn'), $submitButton: $contactUsFormHome.find('.submit-form-btn'), $previousButton: $contactUsFormHome.find('.previous-form-btn'), totalSteps: $contactUsFormHome.find('.custom-form-steps').data('total-steps') });
            ...
            $contactUsFormHome.validate({
                ...
                errorPlacement: function(error, element) {
                    ...
                    } else if (element.is('.select2')) { error.insertAfter(element.siblings('[class*="select2-container"]:eq(0)')); } else if (element.is('.chosen-select')) { error.insertAfter(element.siblings('[class*="chosen-container"]:eq(0)')); } else { error.appendTo(element.closest('.form-group')); }
                },
                submitHandler: function(form) {
                    var $form = $(form);
                    if ($form.hasClass('custom-form') && !CustomForm_IsLastStep($form)) { $form.find('.next-form-btn:visible').trigger('click'); return false; }
                    ...
                    if ($form.hasClass('custom-form') || $form.hasClass('horizontal-custom-form')) { url = "/versions/" + $('#versionNUM').val() + "/include/customFormO.php"; }
            ...
            $contactUsFormHome.find('.f-b-date-timePicker').each(function() {
                ...
                formBuilderCalendar.init({
                    ...
                    calendarSettings: { format: $datePicker.data('date-format'), weekStart: 0, todayBtn: "linked", clearBtn: false, language: languageCode, todayHighlight: true },
...
function CustomForm_IsFillOutAtLeastOneField($form) {
    ...
    $form.find('.form-group').each(function() {
        ...
        if (!isFillOutAtLeastOneField && $inputs.length > 0) { $inputs.each(function() { if ($(this).val().length > 0) { isFillOutAtLeastOneField = true; return false; } }); }
        if (!isFillOutAtLeastOneField && $checkboxs.length > 0 && $checkedCheckboxs.length > 0) { isFillOutAtLeastOneField = true; return false; }
        if (!isFillOutAtLeastOneField && $radios.length > 0 && $checkedRadio.length > 0) { isFillOutAtLeastOneField = true; return false; }
    });
                    ...
                    submitHandler: function(form) {
                        ...
                        $.ajax({
                            ...
                            success: function(respondedMessage) {
                                ...
                                if (!respondedMessage) { $form.find('button:submit').prop('disabled', false); return; }
                                var outPutHTML = GenerateMailingSubscriptionHTML($userEmail.val(), websiteID, w);
                                $form.trigger("reset");
                                bootbox.alert({ title: translations.sent, message: outPutHTML, className: 'contactUsConfirm', backdrop: true, buttons: { ok: { label: 'Update', className: 'btn-primary' } } });
                                ...
                                $.each(countryList, function(countryCode, country) { $('.user-country').append('<option value="' + countryCode + '" ' + (userCountryName == country.name ? 'selected' : '') + '>' + country.name + '</option>'); });
                                $(".phoneIntlInput").intlTelInput({ autoHideDialCode: true, autoPlaceholder: true, geoIpLookup: function(callback) { callback(countryCode); }, initialCountry: "auto", nationalMode: true, numberType: "MOBILE", utilsScript: "/files/frameworks/intl-tel-input-8.5.2/build/js/utils.js" });
                                $(".phoneIntlInput").removeAttr("autocomplete");
                                try { setTimeout(function() { $('.contactUsConfirm').find(".phoneIntlInput").val($('.contactUsConfirm').find(".phoneIntlInput").intlTelInput("getNumber", intlTelInputUtils.numberFormat.INTERNATIONAL)); }, 500); } catch (e) { $('.contactUsConfirm').find(".phoneIntlInput").val($('.contactUsConfirm').find(".phoneIntlInput").val()); }
                                $('.contactUsConfirm').find('button[data-bb-handler=ok]').off('click').on('click', function() {
                                    ...
                                    $.ajax({ type: 'POST', url: '/versions/' + $('#versionNUM').val() + '/include/subscribe-update-info.php', data: { websiteid: websiteID, w: w, email: userEmail, firstName: userFirstName, lastName: userLastName, phone: userPhone, country: userCountry }, success: function(response) {} });
                                });
                                $form.find('button:submit').prop('disabled', false);
                                WizardNotificationUpdate();
                            }
...
function OpenSearchWindow(closeLocation, customPlaceholder) {
    ...
    setTimeout(function() {
        ...
        if (!is_touch_device()) { $('#searchPopup .widget-search-form-keyword').focus(); }
    }, 150);
    $('#searchPopup').submit(function(event) {
        ...
        if ($input.val().length === 0) {
            bootbox.alert({ message: translations.searchInputValidation, className: 'bootbox-search-input-validation' }).on("hidden.bs.modal", function() {
                $form.find('button:submit').prop('disabled', false);
                $input.focus();
            });
            return;
    ...
    $('#popupFloatDivSearch .popupCloseButton').on('click', function() { window.history.replaceState('', 'Title', currentPageUrl); });
    $('#popupFloatDivSearch .cover').on('click', function() { window.history.replaceState('', 'Title', currentPageUrl); });
}
...
function OpenSearchWindowSearchAjax($form, query) {
    ...
    $.ajax({
        ...
        data: $form.serialize(),
        beforeSend: function() { $('#popupFloatDivSearch .result').html('LOADING...'); },
        success: function(data) {
        ...
        if ($('#homepage_full_screen_3_party_video').length !== 0) {
            ...
            if ($videoIframe[0].src.indexOf("youtube.com") > -1) {
                (function() {
                    ...
                    var player;
                    window.onYouTubePlayerAPIReady = function() { player = new YT.Player('homepage_full_screen_3_party_video', { playerVars: { 'autoplay': 1, 'controls': 0, 'autohide': 1, 'wmode': 'opaque', 'loop': 1, 'modestbranding': 1, 'rel': 0, 'showinfo': 0 }, events: { 'onReady': onPlayerReady } }); }
...
function GoToTopButtonInitialize() {
    $(document).on('s123.page.ready', function(event) {
        ...
        $(document).off('S123MagicButton.initialized.GoToTopButton').on('S123MagicButton.initialized.GoToTopButton', function(event) {
            ...
            if (($('html').attr('dir') === 'rtl' && !$('.all-magic-buttons').hasClass('mg-b-icon-position-right')) || $('.all-magic-buttons').hasClass('mg-b-icon-position-left')) { $gotoTop.css({ left: $allMagicButtons.width() + parseInt($allMagicButtons.css('left')) + 10, bottom: '25px' }); } else { $gotoTop.css({ right: $allMagicButtons.width() + parseInt($allMagicButtons.css('right')) + 10, bottom: '25px' }); }
            if (isFreePackage) {
                if ($('html:not(.s123-ads-banner-small) .all-magic-buttons').length !== 0) { $gotoTop.css({ bottom: '75px' }); } else if ($('html[dir="ltr"].in-management.s123-ads-banner-small .all-magic-buttons.mg-b-icon-position-left').length !== 0) { $gotoTop.css({ bottom: '75px' }); } else if ($('html[dir="rtl"].in-management.s123-ads-banner-small .all-magic-buttons').length !== 0) { $gotoTop.css({ bottom: '75px' }); }
                if ($('html[dir="rtl"].in-management.s123-ads-banner-small .all-magic-buttons.mg-b-icon-position-right').length !== 0) { $gotoTop.css({ bottom: '25px' }); }
            }
        });
        $(window).scroll(function() { if ($(window).scrollTop() >= top) { $gotoTop.show(200); } else { $gotoTop.hide(200); } });
    });
}
...
function ContactUsMapObject() {
    $(document).on('s123.page.ready', function(event) {
        $('.s123-module-contact-map, .gmap-container').find('.map-container').each(function(index, upload) {
            ...
            $iframe.parent().click(function(event) { $iframe.css('pointer-events', 'auto'); });
            $iframe.parent().mouseleave(function() { $iframe.css('pointer-events', 'none'); });
        });
    });
}
...
function ActivePopupActionButtonsInPage() {
    ...
    $('[data-toggle="social_menuCallActionIcons"]').off('click').click(function() {
        var $this = $(this);
        if (findBootstrapEnvironment() == 'xs') { var isMobile = 'mobile'; } else { var isMobile = ''; }
    ...
    $('[data-toggle="phone_menuCallActionIcons"]').off('click').click(function() {
        ...
        if (multiPhonesObj.length == 1 && multiPhonesObj[0].note == "") {
            if ((multiPhonesObj[0].type == '1' || multiPhonesObj[0].type == '4') && !isMobile.any()) { openMultiPhonesPopup(); return; }
        ...
        function openMultiPhonesPopup() {
            (function() {
                ...
                $header_phone_content.find('a').each(function() { var $this = $(this); var text_length = $this.text().length; if (text_length > max_text_length) max_text_length = text_length; });
                if (max_text_length > 20) { $header_phone_content.find('.global-contact-details-container').addClass('g-c-d-long-text-handler'); }
            })();
...
function Site123AdButtonInitialize() {
    ...
    var banner_height;
    if (IsWizard()) { $('#showSmallAdOnScroll').addClass('hidden'); return; }
    $(document).on('s123.page.ready', function(event) {
        $html = $('html');
        $showSmallAdOnScroll = $('#showSmallAdOnScroll');
        if ($showSmallAdOnScroll.length === 0) return;
        if ($showSmallAdOnScroll.hasClass('static')) { addHtmlClass(); return; }
        isSmallAds();
        banner_height = $('#showSmallAdOnScroll').outerHeight();
        bannerHandler();
        $(window).scroll(function() { bannerHandler(); });
    });
    $(document).on('s123.page.ready.isSmallAds', function(event) { isSmallAds(); });

    function bannerHandler() {
        var offset = ($html.hasClass('inside_page') || $html.hasClass('rich_page')) ? 0 : 50;
        if ($(window).scrollTop() >= offset) {
            addHtmlClass();
            $showSmallAdOnScroll.css({ bottom: '0' });
        } else {
            removeHtmlClass();
            $showSmallAdOnScroll.css({ bottom: (-1 * banner_height) });
        }
    }
...
function ActiveLazyImageLoad() {
    window.myLazyLoad = new LazyLoad({ elements_selector: 'img.lazyload, .bgLazyload', threshold: 500, callback_enter: function(el) { $(document).trigger('lazyload_enter.image', [$(el)]); } });
    window.iframeLazyload = new LazyLoad({ threshold: 500, elements_selector: '.iframeLazyload' });
    ...
    function fixSafariPromoImages() { if (IsWizard() && /^((?!chrome|android).)*safari/i.test(navigator.userAgent)) { $('.s123-module[data-module-type-num="1000"] .promoImageInline .lazyload:not(.loaded), .s123-module[data-module-type-num="159"] .s123-widget[data-type="imageWidget"] .lazyload:not(.loaded)').each(function(index, img) { LazyLoad.load(img, { callback_loaded: function(el) {} }); }); } }
}
...
function GetMenuPosition() {
    $(document).on('s123.page.ready', function(event) {
        ...
        if (layoutMenuPositionTXT == 'left' || layoutMenuPositionTXT == 'right') { FixMenuTopPosition_SideMenu(); }
...
function MoveFirstSection(sectionNUM) {
    if ($('.promoButtons').data('prevent-btn-action')) { $('.promoButtons').data('prevent-btn-action', false); return; }
    ...
    $('html, body').stop().animate({ scrollTop: ($pages.eq(sectionNUM).offset().top - offset) }, 1250, 'easeInOutExpo');
}
...
function ScrollToModule(fromModuleID, toModuleID) { var offset = findBootstrapEnvironment() != 'xs' ? menuScrollOffset : menuScrollOffset_mobile; var $scrollTo = $('#section-' + toModuleID); if ($scrollTo.length === 0 && fromModuleID != '') $scrollTo = $('#section-' + fromModuleID).next('section'); if ($('html.inside_page').length > 0) { if ($('#w').val() != '') { location.href = '/?w=' + $('#w').val() + '#section-' + toModuleID; } else { location.href = '/#section-' + toModuleID; } } else { if ($scrollTo.length !== 0) { $('html, body').stop().animate({ scrollTop: ($scrollTo.offset().top - offset) }, 1250, 'easeInOutExpo'); } } }
...
function activeDropDownMenusAction() {
    $('.dropdown-submenu > a').off('click.activeDropDownMenusAction').on('click.activeDropDownMenusAction', function(event) { if ($(this).parent().data('is-eCommerce')) { event.preventDefault(); } });
    ...
    $('.navPages').find('.dropdown-submenu').off('click.subMenu mouseenter.subMenu mouseover.subMenu mouseout.subMenu mouseleave.subMenu').on('click.subMenu mouseenter.subMenu mouseover.subMenu mouseout.subMenu mouseleave.subMenu', function(e) {
        ...
        if (eventType == 'mouseenter') { activeDropDownMenusAction_open(e, $this); }
        ...
        if (eventType == 'mouseout') {
            ...
            setTimeout(function() { if ($this.parent('.dropdown-submenu').attr('data-menuSubMenuStillOpen') == 'false') { $this.parent('.dropdown-submenu').removeClass('active').removeClass('open'); } }, 2000);
        }
    });
    ...
    $(document).off('click.subMenu').on('click.subMenu', function(e) { if (dropdownClickFlag == 0 && $('.dropdown-submenu.open').length > 0) { RemoveAllDropDownMenus(); } });
}
...
function activeDropDownMenusAction_open(e, $this) {
    ...
    setTimeout(function() { dropdownClickFlag = 0; }, 1000);
}
...
function AddReturnToManagerBtn() {
    try { if (!window.opener || !window.opener.s123_mobilePreview) return; } catch (err) { return; }
    var html = '';
    html += '<div class="returnToManager text-center">';
    html += '<a>Back to manager</a>';
    html += '</div>';
    $(document.body).append(html);
    $('.returnToManager').css({ 'position': 'fixed', 'bottom': '0px', 'z-index': '100', 'display': 'block', 'height': '53px', 'padding-top': '15px', 'margin-top': '0px', 'padding-bottom': '16px', 'margin-bottom': '0px', 'background-color': '#2196F3', 'width': '100%' });
    $('.returnToManager a').css('color', '#ffffff');
    $('.returnToManager').on('click', function() { window.close(); });
}
...
jQuery(function($) {
    ...
    if (IsWizard()) { $(document).on('s123.pjax.complete', function() { topWindow.Wizard.refresh(); }); }
});
...
function BlockUrlMasking() {
    ...
    if (!$.isNumeric($('#w').val()) && packageNUM < '2') {
        ...
        if (0) { topWindow.location = 'https://' + domain; }
        var websiteID = $('#websiteID').val();
        var topWindowURL = (topWindow && topWindow.location ? topWindow.location : '');
        var referrer = document.referrer;
        $.ajax({ type: 'POST', url: '/manager/UrlMasking.php', data: 'websiteID=' + websiteID + '&topWindowURL=' + topWindowURL + '&referrer=' + referrer });
    }
}
...
var setStickyMenuHandler = function() {
    ...
    that.init = function(settings) {
        ...
        if (that.stickyMenu == 'on') { that.set(); }
    };
    that.set = function() {
        that.$mainNav.affix({ offset: { top: function() { return that.offSetTop } } });
        ...
        that.$mainNav.off('affix.bs.affix').on('affix.bs.affix', function() { if (!$(window).scrollTop()) return false; });
    };
    return that;
}();
    ...
    if (findBootstrapEnvironment() != 'xs' && CheckMenuWidthSpace($place) && $nav.find('>li>a').length > 1) {
        if (CheckMenuWidthSpace($place)) {
            if ($nav.find('.extra-nav-more').length == 0) {
                ...
                if ($place == 'footer') { x += ' ' + S123.s123IconToSvg.getHtml('caret-up', '', '') + '</a> <ul class="site-dropdown-menu dropdown-side-open-up'; } else { x += ' ' + S123.s123IconToSvg.getHtml('caret-down', '', '') + '</a> <ul class="site-dropdown-menu'; }
                x += '"></ul></li>';
                $nav.append(x);
            }
            var $newLIpage = $nav.find(">li").eq(-2).detach().prependTo($nav.find('.extra-nav-more>ul'));
            if ($newLIpage.hasClass('dropdown-submenu') == true) {
                if ($('html').attr('dir') == 'rtl') { $newLIpage.find('.site-dropdown-menu').addClass('dropdown-side-open-left'); } else { $newLIpage.find('.site-dropdown-menu').addClass('dropdown-side-open-right'); }
...
function CheckMenuWidthSpace($place) {
    if ($place == 'header') {
        switch ($('#layoutNUM').val()) {
            case '2':
                if ($('#mainNav .site_container').width() - 50 < $('#top-menu .navPages').outerWidth(true) + $('#top-menu .navActions').outerWidth(true)) { return true; } else { return false; }
                break;
            case '5':
                if ($('.body').outerWidth(false) - 50 < $('#top-menu .navPages').outerWidth(true) + $('#top-menu .navActions').outerWidth(true)) { return true; } else { return false; }
                break;
            case '13':
                if ($('#mainNav .site_container').width() - 50 < $('.navbar-header').outerWidth(true) + $('#top-menu .navPages').outerWidth(true) + $('#top-menu .navActions').outerWidth(true)) { return true; } else { return false; }
                break;
            case '21':
                $('#centerLogo19').remove();
                $('#top-menu').css({ 'padding-right': '0', 'padding-left': '0' });
                if ($('#mainNav .site_container').width() - 50 < $('.navbar-header').outerWidth(true) + $('#top-menu .navPages').outerWidth(true) + $('#mainNav .navActions').outerWidth(true) + 120) { return true; } else { return false; }
                break;
            case '33':
                if ($('#mainNav .site_container').width() < $('.navbar-header').outerWidth(true) + $('#top-menu .navPages').outerWidth(true) + $('#top-menu .navActions').outerWidth(true)) { return true; } else { return false; }
            default:
                if (GetTopMenuWidthByIsContainer() < $('.navbar-header').outerWidth(true) + $('#top-menu .navPages').outerWidth(true) + $('#top-menu .navActions').outerWidth(true)) { return true; } else { return false; }
        }
    }
    if ($place == 'footer') {
        switch ($('#footer_layout').val()) {
            case '2':
                if ($('.global_footer .part1').outerWidth(true) - 100 < $('.global_footer .nav').outerWidth(true)) { return true; } else { return false; }
                break;
            case '1':
            case '3':
            case '4':
                if ($('.global_footer .side2').outerWidth(true) - 100 < $('.global_footer .nav').outerWidth(true)) { return true; } else { return false; }
        }
    }
}
...
function ReduseMenuSizeWhenWeDontHavePlaceHeight_action() {
    ...
    if (findBootstrapEnvironment() != 'xs' && CheckMenuWidthSpaceHeight() && $nav.find('>li>a').length > 1) {
        if (CheckMenuWidthSpaceHeight()) {
            ...
            var $newLIpage = $nav.find(">li").eq(-2).detach().prependTo($nav.find('.extra-nav-more>ul'));
            if ($newLIpage.hasClass('dropdown-submenu') == true) { $newLIpage.find('.site-dropdown-menu').addClass('dropdown-side-open-' + layoutMenuPositionOpenMenuTXT + ''); }
            if ($nav.find('.extra-nav-more').length == 0) { $nav.find('.extra-nav-more').detach().prependTo($nav); }
            ReduseMenuSizeWhenWeDontHavePlaceHeight_action();
        }
    }
...
function ShowMenuAfterReduseSize($place) {
    ...
    if ($('#top-menu').length > 0 && $('#layoutNUM').val() == '21' && $place == 'header') {
        ...
        $('#top-menu .navPages > li').each(function() {
            ...
            if (sumLIofMenu >= menuWidth) { extraPaddingFromSideOne = sumLIofMenu - menuWidth; return false; }
            saveLIplace++;
        });
        if ($('#top-menu .navPages > li').eq(saveLIplace - 1).outerWidth(true) * 0.6 <= (extraPaddingFromSideOne)) { saveLIplace = saveLIplace - 1; }
        ...
        (function() {
            ...
            $logo
                ...
                .on('click.scrollspyFix', function(event) { event.preventDefault(); if (!(IsWizard() && IsHomepage())) { location.href = href; } });
        })();
    ...
    if ($('#mainNavMobile').is(":visible")) {
        if (window.mainNavMobile_page_loaded_icons_states) { window.mainNavMobile_page_loaded_icons_states.show(); } else {
        ...
        if ($('#mainNavMobile .navActions > li:visible').length > 2) { $('#mainNavMobile .navActions > li.header-social-wrapper ').hide(); if ($('#mainNavMobile .navActions > li:visible').length > 2) { $('#mainNavMobile .navActions > li.header-search-wrapper').hide(); if ($('#mainNavMobile .navActions > li:visible').length > 2) { $('#mainNavMobile .navActions > li.header-email-wrapper').hide(); if ($('#mainNavMobile .navActions > li:visible').length > 2) { $('#mainNavMobile .navActions > li.header-address-wrapper').hide(); if ($('#mainNavMobile .navActions > li:visible').length > 2) { $('#mainNavMobile .navActions > li.header-phone-wrapper').hide(); } } } } }
    }
    ...
    $('.navPages .dropdown-submenu > a').off('click.FixMenuTopPosition mouseenter.FixMenuTopPosition').on('click.FixMenuTopPosition mouseenter.FixMenuTopPosition', function(e) {
        ...
        if ($this.length > 0) {
            setTimeout(function() {
                ...
                if (rect.top + rect.height > window.innerHeight && rect.height < window.innerHeight) { $this.css('top', parseInt($this.css('top'), 10) - (rect.top + rect.height - window.innerHeight) - 25); }
                $this.css('opacity', '1');
            }, 100);
        }
    });
    ...
    $menuLinks
        ...
        .on('click.FixMenuTopPosition mouseenter.FixMenuTopPosition', function(e) {
            ...
            setTimeout(function() {
                ...
                if (rect.top + rect.height > window.innerHeight && rect.height < window.innerHeight) { $dropdown.css({ 'bottom': '100%', 'top': 'auto' }); } else { if (rect.top < 0 || rect.bottom < 0) { $dropdown.css({ 'top': '100%', 'bottom': 'auto' }); } }
...
function ResetMoreButton() {
    $('#mainNav #top-menu .navPages, #mainNav #top-menu .navActions, #mainNav #top-menu .headerSocial, #header .header-row, .global_footer .nav').css({ 'opacity': '0' });
    $('#top-menu .navPages .extra-nav-more > ul > li').each(function() {
        var $this = $(this);
        if ($('#mainNav #top-menu').length > 0) { $this.find('.site-dropdown-menu').removeClass('dropdown-side-open-left'); if ($('html').attr('dir') == 'rtl') { $this.find('.fa').removeClass('fa-caret-left').addClass('fa-caret-down'); } else { $this.find('.fa').removeClass('fa-caret-right').addClass('fa-caret-down'); } }
    ...
    if (layoutMenuPositionTXT == 'left' || layoutMenuPositionTXT == 'right') { ReduseMenuSizeWhenWeDontHavePlaceHeight(); } else { ReduseMenuSizeWhenWeDontHavePlace(); }
}
...
function openDivMenuOnLanguageClickAction() {
    ...
    $.each(languageList, function(index, language) { if (language['countryCode'] && language['countryCode'] != '') { content += '<li><a href="' + language['url'] + '"><img src="/files/vendor/flag-icon-css-master/flags/1x1/' + language['countryCode'] + '.svg" style="width:20px;height:14px;">&nbsp;' + language['name'] + '</a></li>'; } else { content += '<li><a href="' + language['url'] + '">' + language['name'] + '</a></li>'; } });
    content += '</ul>';
...
function PageScrollByClick() {
    $(document).on('s123.page.ready.pageScrollByClick', function(event) {
        ...
        $('a.page-scroll').off('click.scrollEvent').on('click.scrollEvent', function(event) {
            var $anchor = $(this);
            $('html, body').stop().animate({ scrollTop: ($($anchor.attr('href')).offset().top - offset) }, 1250, 'easeInOutExpo');
            event.preventDefault();
        });
    });
}

function RefreshScrollSpy() { $(document).on('s123.page.ready.refreshScrollSpy', function(event) { $('body').scrollspy('refresh'); }); };
...
function findBootstrapColPerRow($items) {
    ...
    var col_per_row = 0;
    $items.each(function() { if ($(this).offset().top === first_item_offset_top) { col_per_row += 1; } else { return; } });
    return col_per_row;
}

function buildSmallPopup(popID, title, content, iframeURL, closeEsc, closeEnter, oneColor, closeLocation) {
    if (iframeURL != '') { content = '<iframe id="' + popID + '_iFrame" src="' + iframeURL + '" scrolling="no"></iframe>'; }
    ...
    setTimeout(function() { $('#' + popID + '').find('.content').addClass('open'); }, 100);
    $('#' + popID + ' .cover').click(function() { buildSmallPopup_CloseAction(popID); });
}

function buildSmallPopup_CloseAction(popID) {
    var $popup = $('#' + popID);
    if (popID == 'popupCart' && window.location.pathname == '/-order1/') { $(document).trigger('order1.reload'); }
    setTimeout(function() { $popup.find('.content').removeClass('open'); }, 100);
    setTimeout(function() {
        $('#' + popID).remove();
...
function buildPopup(popID, title, content, iframeURL, closeEsc, closeEnter, oneColor, closeLocation, customClasses) {
    ...
    $('#' + popID).find('.popupCloseButton').click(function() { buildPopup_CloseAction(popID); });
    $('#' + popID + ' .cover').click(function() { buildPopup_CloseAction(popID); });
    if (iframeURL != '') {
        $('#' + popID + '_iFrame').on("load", function() {
            setTimeout(function() {
                ...
                if (!is_touch_device()) { $('#' + popID).find('.page').css({ overflow: 'hidden' }); } else { $('#' + popID).find('.page').css({ overflow: 'auto' }); }
            }, 300);
        });
    }
...
function buildPopup_CloseAction(popID) {
    var $popup = $('#' + popID);
    $popup.find('.page').css({ overflow: 'hidden' });
    $popup.removeClass('open');
    setTimeout(function() { $('#' + popID).remove(); if ($('.popupWin').length == 0) { popupWinScrollAction(0); } }, 700);
            ...
            $.magnificPopup.open({
                ...
                iframe: {
                    ...
                    patterns: {
                        youtube: {
                            index: 'youtube.com/',
                            id: function(url) { var matches = url.match(/[\\?\\&]v=([^\\?\\&]+)/); if (!matches || !matches[1]) return null; return matches[1]; },
                            src: '//www.youtube.com/embed/%id%?autoplay=1'
                        },
                        vimeo: {
                            index: 'vimeo.com/',
                            id: function(url) { var matches = url.match(/(https?:\/\/)?(www.)?(player.)?vimeo.com\/([a-z]*\/)*([0-9]{6,11})[?]?.*/); if (!matches || !matches[5]) return null; return matches[5]; },
                            src: '//player.vimeo.com/video/%id%?autoplay=1'
                        },
                        site123: { index: $GLOBALS['cdn-user-files'], id: function(url) { if (isMobile.any()) url += '&autoplay=0'; return url; }, src: '/include/globalVideoPlayer.php?cad=1&url=%id%' },
                        site123Processing: { index: '/files/images/video-processing.png', id: function(url) { if (isMobile.any()) url += '&autoplay=0'; return url; }, src: '/include/globalVideoPlayer.php?cad=1&url=%id%' }
                    }
                },
            });
        });
    };
    _.addClickEvent = function($el) { $el.on('click.VideoHandler', function() { s123VideoHandler($(this)); }); };
    return _;
}();
...
function s123VideoHandler($obj) {
    ...
    if (player === 'site123') { videoURL = '/include/globalVideoPlayer.php?cad=1&url=' + encodeURIComponent(videoURL) + '&width=' + width + '&height=' + height; }
    if (isMobile.any()) { if (player === 'site123') { videoURL += '&autoplay=false'; } else if (player === 'vimeo') { videoURL = videoURL.replace('autoplay', 'disable-autoplay'); } } else { videoURL += '&autoplay=true'; }
    ...
    $(document).one('S123Resize.start.VideoHandler', function() {
        ...
        $el.imagesLoaded().done(function(instance, image) { $el.css('visibility', 'visible'); });
    ...
    (function() {
        ...
        if ($('#youtube_player_api').length === 0) {
            ...
            var firstScriptTag = document.getElementsByTagName('script')[0];
            firstScriptTag.parentNode.insertBefore(script, firstScriptTag);
            window.onYouTubePlayerAPIReady = function() { youtube_player = YouTubePlayerInitialize(video_id); }
        } else { youtube_player = YouTubePlayerInitialize(video_id); }
...
function WizardNotificationUpdate() { if (IsWizard()) topWindow.Wizard.Notification.update(); }
...
function homepageRandomText() {
    $(document).on('s123.page.ready.homepageRandomText', function(event) {
        holdChangeTextIntervals.forEach(function(element) { clearInterval(element); });
        $('.homepageRandomText').each(function() { homepageRandomTextAction(this, 'no'); });
        $('.homepageRandomTextStop').each(function() { homepageRandomTextAction(this, 'yes'); });
    });
}
...
var ClientZone = function() {
    var CZ = {};
    CZ.init = function() { $(document).on('s123.page.ready', function(event) { CZ.updateClientIcon(); }); };
...
var CartCounter = function() {
    ...
    CC.init = function() {
        ...
        $(document).on('s123.page.ready', function(event) { CC.updateCartIcon(); });
    ...
    CC.oldCustomersSupport = function() {
        ...
        $.ajax({ type: "GET", url: "/versions/" + $('#versionNUM').val() + "/wizard/orders/front/countUserCart.php", data: 'w=' + $('#w').val() + '&websiteID=' + $('#websiteID').val() + '&tranW=' + websiteLanguageCountryFullCode + '&moduleTypeNUM=37' });
        CC.updateCartIcon();
    };
    return CC;
}();
S123.Pjax = function() {
    var that = { active: false, isMobile: null, pjaxSupported: null };
    that.init = function() {
        ...
        document.addEventListener('pjax:complete', function(event) { var is_data_page = that.isDataPage(event.request.response); if (is_data_page) { that.loadDataPageResources(event); } else { that.completeRequestActions(event, 0); } });
    };
    ...
    that.loadDataPageResources = function(event) {
        if (that.isDataResourcesLoaded) { that.completeRequestActions(event, 1); return; }
        ...
        $website_front_data_css.each(function(index, resource) {
            var d1 = $.Deferred();
            $(resource).load(function() { d1.resolve(); })
        ...
        $website_front_data_js.each(function(index, resource) {
            ...
            $.getScript(resource.src)
                .done(function(script, textStatus) { d1.resolve(); });
            resource_deferreds.push(d1);
        });
        $.when.apply($, resource_deferreds).done(function() { that.completeRequestActions(event, 0); });
    };
    that.pjaxInit = function() {
        var pjax_hash_delay = window.location.hash.length !== 0 ? 100 : 0;
        setTimeout(function() {
            that._p = new Pjax({ elements: '.s123-fast-page-load', selectors: ['.s123-js-pjax', '#s123PjaxMainContainer', '#top-menu', "#top-menu-mobile"], scrollRestoration: true, cacheBust: false });
            ...
            that._p.handleResponse = function(responseText, request, href, options) {
                ...
                if (request.responseText.match("<html")) { that._p._handleResponse(responseText, request, href, options); } else {}
            }
            that.active = true;
        }, pjax_hash_delay);
    };
    ...
    that.breadcrumbInitialize = function() { $('.breadcrumb-wrap .breadcrumb.container').find('a').addClass('s123-fast-page-load'); };
    ...
    that.isDataPage = function(str) { var $str = $(str); return $str.find('.s123-page-data').length > 0; };
    ...
    that.triggerComplete = function() { $(document).trigger('s123.pjax.complete'); }
...
function setImagesCDN(path) {
    ...
    try { var Url = new URL(path); var host = Url.host.toLowerCase(); if (host === 'cdn-cms-localhost.f-static.com') { return path; } else if (host === 'second-cdn.f-static.com' || host.indexOf("cdn-cms") !== -1 || host.indexOf("s123-cdn") !== -1) { path = path.replace(host, 'static1.s123-cdn-static-a.com'); } } catch (e) {}
    return path;
}

function tryParseJSON(str) {
    try { var Obj = JSON.parse(str); if (Obj && typeof Obj === "object") { return Obj; } } catch (e) {}
    return false;
}
...
function sharePopover($button, html) {
    ...
    var clipboard = new Clipboard('.popover.share-reply .share-reply-buttons .sharing-url', { target: function() { return document.querySelector('.popover.share-reply .share-reply-buttons .sharing-url'); } });
    $sharingURL.click(function() { $.gritter.add({ title: translations.linkCopiedToClipboard, class_name: 'gritter-success', time: 6000 }); })
    $button.popover({ container: 'body', html: 'true', content: $html, trigger: 'manual', template: '<div class="popover share-reply" role="tooltip" style="max-width: 100%;"><div class="arrow"></div><div class="popover-content"></div></div>', placement: function(popover, button) { return isMobile.any() ? 'auto' : 'top'; } });
    $button.popover('show');
    $button.on('shown.bs.popover', function() {
        $html.find('button').on('click', function() { destroySharePopover(); });
        $(document).on('mousedown.shareDestroyPopover', function(event) { if ($(event.target).closest('.popover.share-reply').length === 0) { destroySharePopover(); } });
    });
...
function Google_reCaptcha(reCaptcha) {
    grecaptcha.ready(function() {
        grecaptcha.render({ 'sitekey': '6LcICoEUAAAAACB-cKbhks2djWsryQxVdJe1eYBi', 'callback': reCaptcha.callback, 'action': reCaptcha.action });
        grecaptcha.execute();
    });
}
...
function showPrice(currency, price) {
    ...
    if (currency.symbolFirst) { var html = '<span data-rel="multiCurrency" dir="ltr"><span data-type="symbol">' + currency.symbol + '</span><span data-type="price">' + price + '</span></span>'; } else { var html = '<span data-rel="multiCurrency" dir="ltr"><span data-type="price">' + price + '</span><span data-type="symbol">' + currency.symbol + '</span></span>'; }
    return html;
}
...
function fitTextToWebsiteScreenWidth(textEle, hasCssVar) {
    ...
    while (bodyWidth < $textEle.containerTextWidth_site123($textEle.css('font-size')) && i <= 50) {
        ...
        fontSize = fontSize - 5;
        if (fontSize > 0) { $textEle.css('font-size', fontSize + 'px'); } else { break; }
        i++;
    }
}
...
var s123MobileMenu = new function() {
    ...
    function addPopupCustomEvents() {
        setTimeout(function() {
            ...
            if (navHeight + actionHeight > screenHeight) { $('#popupFloatDivMenu .navPagesPopup').height(screenHeight - actionHeight - 15); } else { $('#popupFloatDivMenu .navPagesPopup').height(navHeight - 15); }
            $('#popupFloatDivMenu .navPagesPopup .site-dropdown-menu').css('opacity', '1');
        }, 150);
        activeDropDownMenusAction();
        $('#popupFloatDivMenu .navPagesPopup li').not('.dropdown-submenu').find('a').click(function() { buildPopup_CloseAction('popupFloatDivMenu'); });
        $('#popupFloatDivMenu .navPagesPopupActionButtons_part2 a').click(function() { buildPopup_CloseAction('popupFloatDivMenu'); });
        ...
        that.$categories.each(function() {
            ...
            $this.off('click.mobile_categories').on('click.mobile_categories', function(e) {
                ...
                if (!$this.hasClass('active-cat')) {
                    ...
                    that.$selectedCat.children().each(function(index, page) { if ($(page).data('menu-module-id') != menuModuleId || !$(page).hasClass('selected-category')) { $(page).remove(); } else { $(page).find('.site-dropdown-menu').addClass('demo-ul'); } });
    ...
    function animate(action, callBack) {
        ...
        if (action == 'show') {
            ...
            if (that.isRtl) {
                that.$selectedCat.css({ position: 'absolute', top: that.menuTop, right: that.contentOffset, height: that.menuHeight });
                ...
                that.$selectedCat.css({ position: 'absolute', top: that.menuTop, left: that.contentOffset, height: that.menuHeight });
            ...
            that.$selectedCat.css({ position: 'absolute' });
            ...
            that.$ul.stop().animate(animation.ul, that.animation, function() {
                that.$ul.css({ position: '', height: that.menuHeight });
    ...
    function resetBackButtonEvent($category) {
        that.$selectedCat.find('.active-cat').on('click.mobile_categories_back', function(event) {
            event.preventDefault();
            animate('hide', function() { hideCategory($category); });
        });
    }
    ...
    function generateHTML() {
        ...
        } else { that.$source.find('.dropdown-submenu a > i.s123-icon-converter').replaceWith(S123.s123IconToSvg.getHtml('caret-' + (that.isRtl ? 'left' : 'right'), '', '')); }
        ...
        if ($('.header-address-wrapper').length > 0) { html += $('.header-address-wrapper').clone().html(); }
    ...
    function screenResizeHandler() {
        $(window).off('resize.mobile_menu').on('resize.mobile_menu', function() {
            ...
            that.menuTop = that.$ul.offset().top - $(window).scrollTop();
            that.$ul.css({ height: that.menuHeight });
            if (that.isOpened) {
                that.menuTop = that.$selectedCat.offset().top - $(window).scrollTop();
                that.$ul.css({ top: that.menuTop, height: that.menuHeight });
                that.$selectedCat.css({ top: that.menuTop, height: that.menuHeight });
                that.$selectedCat.find('.active-cat-content').css({ maxHeight: that.menuHeight });
                if (that.isRtl) { that.$ul.css({ right: that.contentOffset * -1 }); } else { that.$ul.css({ left: that.contentOffset * -1 }); }
            }
        });
    }
    ...
    function addRemoveAnimationClass(action, elements) {
        switch (action) {
            case 'add':
                for (var i = 0; i < elements.length; i++) { elements[i].addClass('m-m-progress'); }
                break;
            case 'remove':
                for (var i = 0; i < elements.length; i++) { elements[i].removeClass('m-m-progress'); }
                break;
        }
    }
    return that;
...
var ProgressveWebApp = new function() {
    ...
    that.init = function() {
        $(document).on('s123.page.load.progressve_web_app', function(event) {
            ...
            if (!pwaSettings.enableMessage) { window.addEventListener('beforeinstallprompt', function(e) { e.preventDefault(); }); }
        });
    };
    return that;
}();
    ...
    that.getPage = function() {
        ...
        that.request = $.ajax({
            ...
            success: function(data) {
                ...
                if (!data.hasNextPage) { that.destroy(); }
                if (that.ajax.success) that.ajax.success.call(this, data);
                that.request = null;
            }
        });
    };
    that.disable = function() { that.disabled = true; };
    that.enable = function() { that.disabled = false; };
...
S123.popOver = function() {
    ...
    that.init = function(settings) {
        ...
        that.$el.popover({ selector: that.popOverSettings.selector, content: that.popOverSettings.content, html: that.popOverSettings.html, trigger: that.popOverSettings.trigger, template: that.popOverSettings.template, placement: function(popover, input) { if (that.popOverSettings.placementCallBack) { return that.popOverSettings.placementCallBack.call(this); } else { return 'auto'; } } });
        that.$el.on('shown.bs.popover', function(event) {
                that.popOverSettings.content.find('[data-menu-dismiss="popover"]').click(function(event) { if (that.oneTimeUsage) { destroy(); } else { hide(); } });
                $(window).on('mousedown.destroyPopover' + '.' + that.namespace, function(event) { if (that.popOverSettings.trigger === 'manual' && $(event.target).closest(that.elSelector).length > 0) return; if ($(event.target).closest('.' + that.selector).length === 0) { if (that.oneTimeUsage) { destroy(); } else { hide(); } } });
                $(window).one('blur.destroyPopover' + '.' + that.namespace, function(event) { if (that.popOverSettings.trigger === 'manual' && $(event.target).closest(that.elSelector).length > 0) return; if (that.oneTimeUsage) { destroy(); } else { hide(); } });
            ...
            .on('hide.bs.popover' + '.' + that.namespace, function(event) { $(document).trigger('s123_pop_over_wrapper.hide' + '.' + that.namespace); });
    };
...
S123.globalContactEmail = function() {
    ...
    that.init = function() {
        ...
        that.emailBtns.off('click').click(function() {
            ...
            if ($header_email_content.find('a').text().length > 20) { $header_email_content.find('.global-contact-details-container').addClass('g-c-d-long-text-handler'); }
    ...
    that.submitHandler = function() {
        ...
        var customFormMultiSteps = new CustomFormMultiSteps();
        customFormMultiSteps.init({ $form: $form, $nextButton: $form.find('.next-form-btn'), $submitButton: $form.find('.submit-form-btn'), $previousButton: $form.find('.previous-form-btn'), totalSteps: $form.find('.custom-form-steps').data('total-steps') });
                        ...
                        if (data.buttonClickAction && data.buttonClickAction == 'redirect') {
                            if (data.buttonLink_afterSubmit.length > 0) {
                                if (IsWizard()) {
                                    ...
                                    bootbox.alert({ title: translations.previewExternalLinkTitle, message: translations.previewExternalLinkMsg.replace('{{externalLink}}', '<b>' + data.buttonLink_afterSubmit + '</b>'), className: 'externalAlert' });
                                } else {
                                    buildPopup_CloseAction('popupFloatDivSearch');
                            ...
                            $form.next().find(".close-order-thank-you").on("click", function() { buildPopup_CloseAction('popupFloatDivSearch'); });
                        }
                        forms_GoogleRecaptcha.reset();
                        $form.find('button:submit').prop('disabled', false);
                        WizardNotificationUpdate();
...
function CustomFormMultiSteps() {
    ...
    that.initMultiStepsButtons = function() {
        that.$previousButton.off('click').on('click', function() {
            ...
            if (previousStep < 2) { that.$previousButton.hide(); } else { that.$previousButton.fadeIn(350); }
        });
...
function HomepageCountdown() {
    $(document).on('s123.page.ready', function(event) {
        ...
        countdownWidget.init({ $countdown: $countdown, dateInfo: $countdown.data('date-info'), type: $countdown.data('type'), translate: { days: $countdown.data('days'), hours: $countdown.data('hours'), minutes: $countdown.data('minutes'), seconds: $countdown.data('seconds') }, onStop: function() { $container.find('.message').fadeIn(350); } });
    });
}
...
S123.scrollToHandler = function() {
    var _ = {};
    _.init = function() {
        $(document).one('s123.page.load.scrollTo', function(event) { _.scroll(); });
        $(document).on('s123.pjax.complete', function(event) {
            initS123QueryString();
            _.scroll();
        });
    };
    _.scroll = function() { if (!S123.QueryString.scrollTo) return; if (S123.QueryString.scrollTo.indexOf('section') != -1) { ScrollToModule('', S123.QueryString.scrollTo.replace('section-', '')); } };
    return _;
}();
